...
const MapContainer = ({ busData, num, selectedOrg }) => {
  ...
  useEffect(() => {
    ...
    function initializeMap() {
      ...
      //2025 10 15 마커 보간 애니메이션 테스트 함수 추가 추가 줄에는 #표시
      const createOrUpdateMyLocation = ({ latitude, longitude, accuracy, heading }) => {
        ...
        */
        ...
        // 마커가 이미 있다면 부드럽게 이동
        if (window.__myLocationMarker && typeof window.__myLocationMarker.setPosition === 'function') {
          // 기존 좌표와 새 좌표가 다르면 애니메이션 적용
          const prevPos = window.__myLocationMarker.getPosition();
          const prevLat = prevPos.getLat();
          const prevLng = prevPos.getLng();
          if (prevLat !== latitude || prevLng !== longitude) {
            animateMarkerMove(window.__myLocationMarker, latitude, longitude, 10);
          }
          try { window.__myLocationMarker.setImage(makeRotatedMarkerImage(angle)); } catch { }
        } else {
          // 최초 생성
